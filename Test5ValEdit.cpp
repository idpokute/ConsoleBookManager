...
int main() {
  ...
  Screen<<new CLabel(">>>>>> Dialog and ValEdit Tester <<<<<<<<", 0, (console.getCols()-42)/2);
  Screen.draw();  
...
bool ValidPhone(const char* ph , CDialog& owner){
  bool ok = true;
  int i;
  for(i=0;ok && i<3;ok = isdigit(ph[i]), i++);
  ok = ph[i++] == '-';
  for(;ok && i<7;ok = isdigit(ph[i]), i++);
  ok = ph[i++] == '-';
  for(;ok && i<12;ok = isdigit(ph[i]), i++);
  if(ok){
    owner[8].set("");
  }
  else{
